...
interface IProps {
  onSubmit: (data: RedirectRule[]) => void;
  initialValue?: RedirectRule[] | null;
}

const RedirectTool = ({ onSubmit, initialValue }: IProps) => {
  ...
  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const codeUrl = (formData.get("codeUrl") as string)?.trim();
    ...
    let newRules: RedirectRule[];
    
    if (editingRule) {
      // 编辑现有规则
      newRules = rules.map((rule) =>
        rule.id === editingRule.id
          ? { ...rule, codeUrl, redirectUrl }
          : rule
      );
    } else {
      // 添加新规则
      ...
      <div className="w-full">
        ...
        {!isAdding ? (
          ...
          <Form.Root className="w-full" onSubmit={handleSubmitForm}>
            ...
            <Form.Field className="my-2 w-full" name="redirectUrl">
              <Form.Control asChild>
                <input
                  name="redirectUrl"
