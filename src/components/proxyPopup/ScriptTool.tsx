...
interface IProps {
  onSubmit: (data: ScriptRule[]) => void;
  initialValue?: ScriptRule[] | null;
}

const ScriptTool = ({ onSubmit, initialValue }: IProps) => {
  const [rules, setRules] = useState<ScriptRule[]>(initialValue || []);
  const [editingRule, setEditingRule] = useState<ScriptRule | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const scriptUrl = (formData.get("scriptUrl") as string)?.trim();
    const replacementContent = (
      formData.get("replacementContent") as string
    )?.trim();
    ...
    let newRules: ScriptRule[];

    if (editingRule) {
      // 编辑现有规则
      newRules = rules.map((rule) =>
        rule.id === editingRule.id
          ? { ...rule, scriptUrl, replacementContent, isGlobal: true, specificUrl: undefined }
          : rule
      );
    } else {
      // 添加新规则
      ...
      <div className="w-full">
        ...
        {!isAdding ? (
          ...
          <Form.Root className="w-full" onSubmit={handleSubmit}>
            ...
            <Form.Field className="my-2 w-full" name="replacementContent">
              <Form.Control asChild>
                <textarea
