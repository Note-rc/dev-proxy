...
// 路由替换配置组件
const ProxyConfig: React.FC<{ initialValue: ProxyRule[] }> = ({
  initialValue,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const updated = rules.map((r) =>
        r.id === editingId
          ? { ...r, ...formData, isGlobal: true, specificUrl: undefined }
          : r,
      );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4">
        {rules.map((rule) => (
          <div
            key={rule.id}
            className={`p-4 rounded-lg border-2 transition-all ${rule.enabled ? "border-blue-200 bg-blue-50" : "border-gray-200 bg-gray-50"}`}
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${rule.enabled ? "translate-x-7" : "translate-x-1"}`}
                    />
                  </button>
                </div>
...
// 脚本替换配置组件
const ScriptConfig: React.FC<{ initialValue: ScriptRule[] }> = ({
  initialValue,
}) => {
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4">
        {rules.map((rule) => (
          <div
            key={rule.id}
            className={`p-4 rounded-lg border-2 transition-all ${rule.enabled ? "border-green-200 bg-green-50" : "border-gray-200 bg-gray-50"}`}
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-gray-600 font-mono text-xs">
                      ...
                      {rule.replacementContent.length > 80 ? "..." : ""}
                    </span>
                  </div>
                </div>
              </div>
...
// JS重定向配置组件
const RedirectConfig: React.FC<{ initialValue: RedirectRule[] }> = ({
  initialValue,
}) => {
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4">
        {rules.map((rule) => (
          <div
            key={rule.id}
            className={`p-4 rounded-lg border-2 transition-all ${rule.enabled ? "border-purple-200 bg-purple-50" : "border-gray-200 bg-gray-50"}`}
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
...
// 请求头配置组件
const HeaderConfigPanel: React.FC<{ initialValue: HeaderRule[] }> = ({
  initialValue,
}) => {
  ...
  const saveRules = async (newRules: HeaderRule[]) => {
    await chromeStore.set("headerConfig", newRules);
    setRules(newRules);
  };
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4">
        {rules.map((rule) => (
          <div
            key={rule.id}
            className={`p-4 rounded-lg border-2 transition-all ${rule.enabled ? "border-orange-200 bg-orange-50" : "border-gray-200 bg-gray-50"}`}
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                ...
                <div className="space-y-1 text-sm">
                  <div>
                    ...
                    <span className="text-gray-600">{rule.headerValue}</span>
                  </div>
                  {rule.urlPattern && (
                    <div>
                      <span className="font-semibold text-gray-700">
...
// 代理服务器配置组件
const ProxyServerConfig: React.FC<{ initialValue: ProxyConfig | null }> = ({
  initialValue,
}) => {
...
const ConfigPage: React.FC = () => {
  ...
  const [proxyServerConfig, setProxyServerConfig] =
    useState<ProxyConfig | null>(null);
        ...
        <div className="grid grid-cols-1 gap-6">
          {configs.map((config) => (
            <ConfigCard
              ...
              onToggle={() =>
                setExpandedCard(expandedCard === config.id ? null : config.id)
              }
            >
              {config.component}
            </ConfigCard>
